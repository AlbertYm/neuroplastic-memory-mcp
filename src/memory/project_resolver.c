...
int cbm_project_uuid_v5(const char *canonical_path, char out[CBM_PROJECT_UUID_SIZE]) {
    ...
    if (!canonical_path || !canonical_path[0] || !out ||
        uuid_parse(CBM_STAGE14_NAMESPACE_UUID, ns) != 0) return -1;
    stage14_sha1_t sha;
    sha1_init(&sha);
...
int cbm_project_canonicalize_path(const char *path_or_file_uri, char *out, size_t out_size) {
    ...
    if (!path_or_file_uri || !out || out_size == 0 ||
        decode_file_uri(path_or_file_uri, decoded, sizeof(decoded)) != 0) return -1;
#ifdef _WIN32
    wchar_t input_w[4096], full_w[4096];
    int wn = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, decoded, -1, input_w, 4096);
    if (wn <= 0 || !_wfullpath(full_w, input_w, 4096)) return -1;
    HANDLE handle=CreateFileW(full_w,FILE_READ_ATTRIBUTES,FILE_SHARE_READ|FILE_SHARE_WRITE|FILE_SHARE_DELETE,
                              NULL,OPEN_EXISTING,FILE_FLAG_BACKUP_SEMANTICS,NULL);
    if(handle!=INVALID_HANDLE_VALUE){
        wchar_t final_w[4096];DWORD n=GetFinalPathNameByHandleW(handle,final_w,4096,FILE_NAME_NORMALIZED);
        CloseHandle(handle);
        if(n>0&&n<4096){
            const wchar_t *resolved=final_w;
            if(wcsncmp(resolved,L"\\\\?\\UNC\\",8)==0){swprintf(full_w,4096,L"\\\\%ls",resolved+8);}
            else if(wcsncmp(resolved,L"\\\\?\\",4)==0)swprintf(full_w,4096,L"%ls",resolved+4);
            else swprintf(full_w,4096,L"%ls",resolved);
        }
    }
    ...
    while (len > 3 && (full_w[len - 1] == L'\\' || full_w[len - 1] == L'/')) full_w[--len] = 0;
    int bytes = WideCharToMultiByte(CP_UTF8, 0, full_w, -1, out, (int)out_size, NULL, NULL);
    return bytes > 0 ? 0 : -1;
#else
    char full[PATH_MAX];
    if (!realpath(decoded, full)) {
        if (decoded[0] == '/') snprintf(full, sizeof(full), "%s", decoded);
        else {
            char cwd[PATH_MAX];
    ...
    for (size_t i = 0; i <= len; i++) out[i] = (char)tolower((unsigned char)full[i]);
    return 0;
#endif
}
...
#else
    struct stat st;
    if (stat(canonical_path, &st) != 0) return -1;
...
int cbm_project_resolve(const char *path_or_file_uri, const char *display_name,
    ...
    if (cbm_project_canonicalize_path(path_or_file_uri, out->canonical_path,
        ...
        cbm_stage7_sha256_hex(out->canonical_path, strlen(out->canonical_path), out->path_hash) != 0 ||
        cbm_project_uuid_v5(out->canonical_path, out->project_uuid) != 0) return -1;
    ...
    if (source_fingerprint && source_fingerprint[0]) {
        if (strlen(source_fingerprint) != 64) return -1;
        for (size_t i = 0; i < 64; i++) if (hex_nibble(source_fingerprint[i]) < 0) return -1;
        snprintf(out->source_fingerprint, sizeof(out->source_fingerprint), "%s", source_fingerprint);
    }
#ifdef _WIN32
