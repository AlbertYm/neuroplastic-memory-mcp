...
static CBMLogLevel g_log_level = CBM_LOG_WARN;
...
static const char *log_safe_value(const char *key, const char *value) {
    (void)key;
    if (log_value_is_windows_absolute_path(value)) {
...
void cbm_log(CBMLogLevel level, const char *msg, ...) {
    ...
    if (g_log_format == CBM_LOG_FORMAT_JSON) {
        ...
        for (;;) {
            ...
            if (log_key_forbidden(key)) {
                continue;
            }
            const char *safe_val = log_safe_value(key, val);
            append_char(line_buf, sizeof(line_buf), &pos, ',');
            append_json_string(line_buf, sizeof(line_buf), &pos, key);
            append_char(line_buf, sizeof(line_buf), &pos, ':');
            append_json_string(line_buf, sizeof(line_buf), &pos, safe_val);
        }
        append_char(line_buf, sizeof(line_buf), &pos, '}');
    } else {
        append_raw(line_buf, sizeof(line_buf), &pos, "level=");
        ...
        for (;;) {
            ...
            append_char(line_buf, sizeof(line_buf), &pos, '=');
            append_text_atom(line_buf, sizeof(line_buf), &pos, safe_val);
        }
    }
    va_end(args);
