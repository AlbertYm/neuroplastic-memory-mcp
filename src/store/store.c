...
static int store_result_from_sqlite(int rc) {
    int primary = rc & 0xff;
    if (primary == SQLITE_BUSY || primary == SQLITE_LOCKED) {
        return CBM_STORE_BUSY;
    }
    if (primary == SQLITE_INTERRUPT) {
        return CBM_STORE_TIMEOUT;
    }
    if (primary == SQLITE_CORRUPT || primary == SQLITE_NOTADB || primary == SQLITE_IOERR) {
        return CBM_STORE_RECOVERY_REQUIRED;
    }
    return CBM_STORE_ERR;
}

const char *cbm_store_result_code_name(int result_code) {
    switch (result_code) {
    case CBM_STORE_OK:
        return "OK";
    case CBM_STORE_NOT_FOUND:
        return "NOT_FOUND";
    case CBM_STORE_BUSY:
        return "DB_BUSY";
    case CBM_STORE_TIMEOUT:
        return "DB_TIMEOUT";
    case CBM_STORE_RECOVERY_REQUIRED:
        return "DB_RECOVERY_REQUIRED";
    case CBM_STORE_BACKUP_VERIFY_FAILED:
...
static int exec_sql(cbm_store_t *s, const char *sql) {
    ...
    if (rc != SQLITE_OK) {
        ...
        sqlite3_free(err);
        return store_result_from_sqlite(rc);
    }
    return CBM_STORE_OK;
}
...
static cbm_store_t *store_open_internal(const char *path, bool in_memory) {
    ...
    if (configure_pragmas(s, in_memory, false) != CBM_STORE_OK || init_schema(s) != CBM_STORE_OK ||
        ...
        /* Local fork: memory_* tables live in src/memory/ with their own
         * versioned migration chain (PRAGMA user_version, unused upstream). */
        cbm_memory_run_migrations(s) != CBM_STORE_OK) {
        sqlite3_close(s->db);
