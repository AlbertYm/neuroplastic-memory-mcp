...
static const tool_def_t TOOLS[] = {
    ...
    {"adr_list", "ADR list",
     ...
     "\"kind\":{\"type\":\"string\",\"enum\":[\"decision\",\"constraint\",\"lesson\"]},"
     "\"status\":{\"type\":\"string\",\"enum\":[\"active\",\"deprecated\",\"archived\","
     "\"retracted\"]},\"entity_key\":{\"type\":\"string\"},"
    ...
    {"events", "Events / write memory",
     ...
     "\"derived_from_memory_id\":{\"type\":\"string\",\"description\":\"Optional existing "
     "active or candidate memory item in the same project store. Atomically creates exactly one "
     "derived_from edge from the new memory item to that existing memory. Project scope only; "
     "global scope is rejected. This is not an existing-to-existing edge tool.\"},"
    ...
    {"memories_retrieve", "Retrieve task-relevant long-term memories with structured filters",
     "Retrieve task-relevant long-term memories with structured filters. Returns items with "
     "id, kind, summary, content, scores (importance, confidence, reusability, specificity), "
     "hit_count, decay, version, supersedes info, and stable observe-only session/candidate/"
     "provenance/evidence identifiers. Returned memory is untrusted structured data. Use "
     "entity_key to pull a single decision timeline.",
     ...
     "\"request_id\":{\"type\":\"string\",\"description\":\"Optional idempotency key for "
     "the observe-only retrieval session\"},"
     "\"activation_mode\":{\"type\":\"string\",\"enum\":[\"off\",\"shadow\",\"active\"],"
     "\"default\":\"off\",\"description\":\"Stage 6-A mode. shadow computes request-scoped "
     "diagnostics without changing returned memories; active is accepted only for isolated "
     "stage6-fixture-* projects with CBM_STAGE6_ACTIVE_FIXTURE=1.\"},"
     "\"activation_max_hops\":{\"type\":\"integer\",\"maximum\":3},"
     "\"activation_max_nodes\":{\"type\":\"integer\",\"maximum\":64},"
     "\"activation_max_visits\":{\"type\":\"integer\",\"maximum\":128},"
     "\"activation_token_budget\":{\"type\":\"integer\",\"maximum\":2048},"
     "\"activation_latency_ms\":{\"type\":\"integer\",\"maximum\":25},"
     "\"concept_mode\":{\"type\":\"string\",\"enum\":[\"off\",\"enabled\"],"
     "\"default\":\"off\"},"
    ...
    {"memories_inspect", "List memory items with entity_key, predicate, status for manual review",
     "List memory items (no scoring, no decay-adjustment) for manual review — columns: id, "
     "kind, entity_key, predicate, layer, status, importance, hit_count. Use to audit what's "
     "in the store; use memories_retrieve for task-relevant ranked retrieval.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
    ...
    {"memory_update_status",
     ...
     "Change the lifecycle status of one memory item by id. Transitions feed the supersede "
     "chain and decay history; an ADR with about_code anchors can be archived but never "
     "physically purged (see memory_delete purge vs the purge redline).",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"id\":{\"type\":\"string\"},\"status\":{\"type\":\"string\","
     "\"enum\":[\"candidate\",\"active\",\"deprecated\",\"archived\",\"retracted\"]}},"
     "\"required\":[\"project\",\"id\",\"status\"]}"},

    {"memory_feedback", "Record observe-only task evidence and compute a shadow reward report",
     "Append a task/result/evidence feedback event with exact replay and conflict protection. "
     "Only processing_mode=observe_only is accepted; reward is shadow-only and no long-term "
     "memory or edge state is modified. Legacy project/id/feedback calls fail closed.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"processing_mode\":{\"type\":\"string\",\"enum\":[\"observe_only\"]},"
     "\"event_id\":{\"type\":\"string\"},\"task_id\":{\"type\":\"string\"},"
     "\"task_type\":{\"type\":\"string\",\"enum\":[\"build\",\"test\","
     "\"static_check\",\"runtime\",\"user_task\",\"health_check\"]},"
     "\"session_id\":{\"type\":\"string\"},\"candidate_id\":{\"type\":\"string\"},"
     "\"injection_id\":{\"type\":\"string\"},\"usage_id\":{\"type\":\"string\"},"
     "\"result_id\":{\"type\":\"string\"},\"result_type\":{\"type\":\"string\","
     "\"enum\":[\"build\",\"test\",\"static_check\",\"runtime\","
     "\"user_confirmation\",\"health_check\"]},\"result_status\":{\"type\":\"string\","
     "\"enum\":[\"succeeded\",\"failed\",\"cancelled\",\"pending\"]},"
     "\"result_ref\":{\"type\":\"string\"},\"result_hash\":{\"type\":\"string\"},"
     "\"result_payload\":{\"type\":\"string\"},\"evidence_id\":{\"type\":\"string\"},"
     "\"evidence_trust\":{\"type\":\"string\",\"enum\":[\"external_verified\","
     "\"explicit_user\",\"model_self_report\"]},\"evidence_state\":{\"type\":\"string\","
     "\"enum\":[\"valid\",\"invalid\",\"expired\",\"withdrawn\"]},"
     "\"evidence_source\":{\"type\":\"string\",\"enum\":[\"build\",\"test\","
     "\"static_check\",\"runtime\",\"user\",\"model\"]},"
     "\"evidence_ref\":{\"type\":\"string\"},\"evidence_hash\":{\"type\":\"string\"},"
     "\"evidence_payload\":{\"type\":\"string\"},\"action\":{\"type\":\"string\","
     "\"enum\":[\"confirm\",\"reject\",\"correct\",\"withdraw\"]},"
     "\"edge_id\":{\"type\":\"string\"},\"supersedes_event_id\":{\"type\":\"string\"},"
     "\"algorithm_version\":{\"type\":\"string\"},"
     "\"config_version\":{\"type\":\"integer\",\"minimum\":0}},"
     "\"required\":[\"project\",\"processing_mode\",\"event_id\",\"task_id\","
     "\"task_type\",\"session_id\",\"candidate_id\",\"usage_id\",\"result_id\","
     "\"result_type\",\"result_status\",\"result_ref\",\"result_hash\","
     "\"result_payload\",\"evidence_id\",\"evidence_trust\",\"evidence_state\","
     "\"evidence_source\",\"evidence_ref\",\"evidence_hash\",\"evidence_payload\","
     "\"action\",\"algorithm_version\",\"config_version\"]}"},

    {"memory_reinforcement_replay", "Replay feedback or control task evolution",
     "Preserves the fixed Stage 8 replay contract. The Stage 14 branch exposes a read-only "
     "bounded-canary task preview and an explicit apply bound to both the task manifest and "
     "the outer production authorization manifest. Active production mode is forbidden.",
     "{\"type\":\"object\",\"oneOf\":[{\"type\":\"object\","
     "\"additionalProperties\":false,\"properties\":{"
     "\"project\":{\"type\":\"string\"},"
     "\"mode\":{\"type\":\"string\",\"enum\":[\"off\",\"shadow\",\"active\"]},"
     "\"algorithm_version\":{\"type\":\"string\",\"const\":"
     "\"stage8-edge-reinforcement-v1\"},\"config_version\":{\"type\":\"integer\","
     "\"const\":1}},\"required\":[\"project\",\"mode\",\"algorithm_version\","
     "\"config_version\"]},{\"type\":\"object\",\"additionalProperties\":false,"
     "\"properties\":{\"action\":{\"type\":\"string\",\"const\":\"preview\"},"
     "\"mode\":{\"type\":\"string\",\"const\":\"bounded_canary\"},"
     "\"project\":{\"type\":\"string\",\"minLength\":1},"
     "\"run_id\":{\"type\":\"string\",\"minLength\":1},"
     "\"task_id\":{\"type\":\"string\",\"minLength\":1},"
     "\"idempotency_key\":{\"type\":\"string\",\"minLength\":1},"
     "\"max_evolution_events\":{\"type\":\"integer\",\"minimum\":1,\"maximum\":17},"
     "\"max_cross_project_edges\":{\"type\":\"integer\",\"minimum\":0,\"maximum\":16}},"
     "\"required\":[\"action\",\"mode\",\"project\",\"run_id\",\"task_id\","
     "\"idempotency_key\",\"max_evolution_events\",\"max_cross_project_edges\"]},"
     ...
     "\"authorization_manifest_path\":{\"type\":\"string\",\"minLength\":1},"
     "\"authorization_manifest_sha256\":{\"type\":\"string\","
     "\"pattern\":\"^[0-9a-f]{64}$\"}},\"required\":[\"action\",\"mode\","
     "\"project\",\"run_id\",\"task_id\",\"idempotency_key\","
     "\"max_evolution_events\",\"max_cross_project_edges\",\"manifest_path\","
     "\"manifest_sha256\",\"authorization_manifest_path\","
     "\"authorization_manifest_sha256\"]}]}"},

    {"memory_edge_lifecycle_migrate", "Apply the guarded additive Stage 9 edge schema",
     "Apply the independent Stage 9 edge-lifecycle schema. Requires the fixed fixture guard or "
     "the three production canary environment fields. It never alters Stage 8 history.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"algorithm_version\":{\"type\":\"string\",\"const\":\"stage9-edge-lifecycle-v1\"},"
     "\"policy_sha256\":{\"type\":\"string\",\"const\":"
     ...
     "\"required\":[\"project\",\"algorithm_version\",\"policy_sha256\","
     "\"policy_version\",\"config_version\"]}"},

    {"memory_edge_maintenance", "Preview or apply deterministic edge lifecycle decisions",
     "Compute explainable conductance and active/cold/archived decisions. Off, shadow, and "
     "dry_run write nothing. Active requires a frozen manifest and canary guard.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"mode\":{\"type\":\"string\",\"enum\":[\"off\",\"shadow\",\"dry_run\",\"active\"]},"
     "\"run_id\":{\"type\":\"string\"},\"as_of_ms\":{\"type\":\"integer\",\"minimum\":0},"
     "\"algorithm_version\":{\"type\":\"string\",\"const\":\"stage9-edge-lifecycle-v1\"},"
     "\"policy_sha256\":{\"type\":\"string\",\"const\":"
     "\"8ba8ca0610997d4ffd6a0d2e5e98460a7ec3bf7b2fae2bdfaef9fd41d5802b0c\"},"
     "\"policy_version\":{\"type\":\"integer\",\"const\":1},"
     ...
     "\"required\":[\"project\",\"mode\",\"run_id\",\"as_of_ms\","
     "\"algorithm_version\",\"policy_sha256\",\"policy_version\",\"config_version\"]}"},

    {"memory_edge_restore", "Preview or restore archived edges through append-only audit",
     "Batch-preview or restore cold/archived edges to active. Disabled edges remain blocked. "
     "Active restore requires the same frozen manifest and canary guard as maintenance.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"mode\":{\"type\":\"string\",\"enum\":[\"shadow\",\"dry_run\",\"active\"]},"
     "\"run_id\":{\"type\":\"string\"},\"as_of_ms\":{\"type\":\"integer\",\"minimum\":0},"
    ...
    {"memory_concept_generate", "Evaluate or generate proposed concept candidates",
     "Apply the guarded additive Stage 10 schema, evaluate deterministic candidates, or generate "
     "proposed-only candidates. Off, shadow, and dry_run never write long-term Stage 10 state.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"store\":{\"type\":\"string\",\"const\":\"project-memory\"},"
     "\"operation\":{\"type\":\"string\",\"enum\":[\"migrate\",\"evaluate\",\"generate\"]},"
     "\"mode\":{\"type\":\"string\",\"enum\":[\"off\",\"shadow\",\"dry_run\",\"active\"]},"
     "\"run_id\":{\"type\":\"string\"},\"idempotency_key\":{\"type\":\"string\"},"
     "\"algorithm_version\":{\"type\":\"string\",\"const\":\"stage10-concept-growth-v1\"},"
     "\"policy_sha256\":{\"type\":\"string\",\"const\":"
     "\"38e32e2c0fbe224db8e6ca04fb1aff58b38cbc7255904b70563c3d48018746b4\"},"
     "\"policy_version\":{\"type\":\"integer\",\"const\":1},"
     "\"config_version\":{\"type\":\"integer\",\"const\":1},"
     "\"generator_version\":{\"type\":\"string\",\"const\":\"deterministic-local-v1\"},"
     "\"manifest_path\":{\"type\":\"string\"},\"manifest_sha256\":{\"type\":\"string\"}},"
     "\"required\":[\"project\",\"store\",\"operation\",\"mode\",\"algorithm_version\","
     "\"policy_sha256\",\"policy_version\",\"config_version\",\"generator_version\"]}"},

    {"memory_concept_review", "Review one proposed concept candidate",
     "Append an explicit edit, approval, rejection, or withdrawal. Production review requires "
     "content-level user confirmation and never runs automatically.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"store\":{\"type\":\"string\",\"const\":\"project-memory\"},"
     "\"candidate_id\":{\"type\":\"string\"},\"action\":{\"type\":\"string\","
     "\"enum\":[\"approve\",\"edit\",\"reject\",\"withdraw\"]},"
     "\"idempotency_key\":{\"type\":\"string\"},\"content_text\":{\"type\":\"string\"},"
     "\"related_candidate_id\":{\"type\":\"string\"},"
     "\"explicit_user_confirmed\":{\"type\":\"boolean\",\"const\":true}},"
     "\"required\":[\"project\",\"store\",\"candidate_id\",\"action\","
     "\"idempotency_key\",\"explicit_user_confirmed\"]}"},

    {"memory_concept_inspect", "Inspect candidate provenance and append-only history",
     "Read one concept candidate, its versions, source count, review-derived state, node, and "
     "relation counts. Candidate text is explicitly marked as untrusted data.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"store\":{\"type\":\"string\",\"const\":\"project-memory\"},"
     "\"candidate_id\":{\"type\":\"string\"}},"
     "\"required\":[\"project\",\"store\",\"candidate_id\"]}"},

    {"memory_observe_injection", "Record an observe-only context injection decision",
     "Record a classifier-gated injection by retrieval session and candidate. The interface "
     "accepts hashes and audit metadata, never executable memory text. Exact event replay is "
     "idempotent; a changed payload conflicts without side effects.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"event_id\":{\"type\":\"string\"},\"session_id\":{\"type\":\"string\"},"
     "\"candidate_id\":{\"type\":\"string\"},\"injection_index\":{\"type\":\"integer\"},"
     "\"target\":{\"type\":\"string\"},\"content_hash\":{\"type\":\"string\"},"
     "\"token_count\":{\"type\":\"integer\"},"
     "\"classifier_status\":{\"type\":\"string\",\"enum\":[\"pass\",\"error\"]},"
     "\"classification\":{\"type\":\"string\",\"enum\":[\"safe\",\"prompt_injection\","
     "\"secret\",\"pii\",\"canary\",\"rule_override\"]}},"
     "\"required\":[\"project\",\"event_id\",\"session_id\",\"candidate_id\","
     "\"injection_index\",\"target\",\"content_hash\",\"token_count\","
     "\"classifier_status\",\"classification\"]}"},

    {"memory_observe_usage", "Record observe-only memory usage attribution",
     "Record used, ignored, rejected, contradicted, or uncertain attribution for one candidate "
     "and optional injection. Session/candidate/injection mismatches fail closed. Exact event "
     "replay is idempotent; a changed payload conflicts without side effects.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"event_id\":{\"type\":\"string\"},\"session_id\":{\"type\":\"string\"},"
     "\"candidate_id\":{\"type\":\"string\"},\"injection_id\":{\"type\":\"string\"},"
     "\"outcome\":{\"type\":\"string\",\"enum\":[\"used\",\"ignored\",\"rejected\","
     "\"contradicted\",\"uncertain\"]},\"evidence_type\":{\"type\":\"string\"},"
     "\"evidence_ref\":{\"type\":\"string\"},\"evidence_hash\":{\"type\":\"string\"}},"
     "\"required\":[\"project\",\"event_id\",\"session_id\",\"candidate_id\","
     "\"outcome\",\"evidence_type\",\"evidence_ref\"]}"},

    {"memory_delete", "Soft-delete or restore a memory item; physical deletion is disabled",
    ...
    {"admin_consolidate", "Run the deterministic memory consolidation pass for candidate items",
     "Run the deterministic consolidation pass over candidate memories for a project: merges "
     "near-duplicate candidates, recomputes scores from graph signals. Returns how many were "
     "processed. Idempotent; safe to run periodically.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"limit\":{\"type\":\"integer\"}},\"required\":[\"project\"]}"},

    {"admin_decay", "Run explainable decay and archive stale active memories",
     "Run the decay pass: lowers retrieval scores of active memories by age and hit_count, "
     "archives stale ones into candidate/deprecated. Returns how many were processed. "
     "Idempotent; safe to run periodically.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"limit\":{\"type\":\"integer\"}},\"required\":[\"project\"]}"},

    {"memory_health", "Report long-term memory MVP health counters",
     "Report health counters for a project's memory store: total items by status, by kind, "
     "decay distribution, anchor coverage, FTS index status. Use to gauge store quality before "
     "and after consolidation/decay passes.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"}},"
     "\"required\":[\"project\"]}"},

    {"describe_tool", "Return the full usage guide for one tool",
    ...
    {"memory_security_check", "Classify untrusted memory content without writing",
     ...
     "only policy metadata, category, action, stable reason codes, SHA256 and byte length; the "
     "input is never echoed or persisted.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"store\":{\"type\":\"string\",\"const\":\"project-memory\"},"
    ...
    {"memory_task_migrate", "Apply the guarded additive Stage 12 task schema",
     ...
     "\"required\":[\"project\",\"manifest_path\",\"manifest_sha256\",\"idempotency_key\"]}"},
    ...
    {"manager_maintenance_control", "Control guarded maintenance",
     ...
     "the bounded request. Production run/resume requires authorization, edge, and concept "
     "manifest bindings. Production pause fails closed until a core audit API exists.",
     "{\"type\":\"object\",\"oneOf\":[{\"type\":\"object\","
     "\"additionalProperties\":false,\"properties\":{"
     "\"action\":{\"type\":\"string\",\"enum\":[\"pause\",\"resume\",\"dry_run\"]},"
     "\"scope\":{\"type\":\"string\",\"const\":\"isolated_mock\"}},"
     "\"required\":[\"action\",\"scope\"]},{\"type\":\"object\","
     "\"additionalProperties\":false,\"properties\":{"
     "\"action\":{\"type\":\"string\",\"const\":\"preview\"},"
     "\"scope\":{\"type\":\"string\",\"const\":\"production\"},"
     "\"mode\":{\"type\":\"string\",\"const\":\"bounded_canary\"},"
     "\"project\":{\"type\":\"string\",\"minLength\":1},"
     "\"owner_id\":{\"type\":\"string\",\"minLength\":1},"
     "\"run_id\":{\"type\":\"string\",\"minLength\":1},"
     "\"idempotency_key\":{\"type\":\"string\",\"minLength\":1},"
     "\"frozen_as_of_ms\":{\"type\":\"integer\",\"minimum\":1},"
     "\"limit\":{\"type\":\"integer\",\"minimum\":1,\"maximum\":1000},"
     "\"budget_seconds\":{\"type\":\"integer\",\"minimum\":1,\"maximum\":30}},"
     "\"required\":[\"action\",\"scope\",\"mode\",\"project\",\"owner_id\","
     "\"run_id\",\"idempotency_key\",\"frozen_as_of_ms\",\"limit\","
     "\"budget_seconds\"]},{\"type\":\"object\",\"additionalProperties\":false,"
     "\"properties\":{"
     "\"action\":{\"type\":\"string\",\"enum\":[\"run\",\"resume\",\"pause\"]},"
     "\"scope\":{\"type\":\"string\",\"const\":\"production\"},"
     "\"mode\":{\"type\":\"string\",\"const\":\"bounded_canary\"},"
     "\"project\":{\"type\":\"string\",\"minLength\":1},"
     "\"owner_id\":{\"type\":\"string\",\"minLength\":1},"
     ...
     "\"limit\":{\"type\":\"integer\",\"minimum\":1,\"maximum\":1000},"
     "\"budget_seconds\":{\"type\":\"integer\",\"minimum\":1,\"maximum\":30},"
     "\"edge_manifest_path\":{\"type\":\"string\",\"minLength\":1},"
     "\"edge_manifest_sha256\":{\"type\":\"string\",\"pattern\":\"^[0-9a-f]{64}$\"},"
     "\"concept_manifest_path\":{\"type\":\"string\",\"minLength\":1},"
     "\"concept_manifest_sha256\":{\"type\":\"string\",\"pattern\":\"^[0-9a-f]{64}$\"},"
     "\"authorization_manifest_path\":{\"type\":\"string\",\"minLength\":1},"
     "\"authorization_manifest_sha256\":{\"type\":\"string\","
     "\"pattern\":\"^[0-9a-f]{64}$\"}},\"required\":[\"action\",\"scope\","
     "\"mode\",\"project\",\"owner_id\",\"run_id\",\"idempotency_key\","
...
/* Codex consumes only the first tools/list page. Keep the smallest usable
 * memory and task-closeout loop visible there without displacing the graph
 * tools already exposed on that page; the remaining tools continue through
 * the same cursor-based inventory. */
static const char *MCP_FIRST_PAGE_COMPAT_TOOLS[] = {
    "events",
    "index_repository",
    "memories_retrieve",
    "memory_task_status",
...
static const size_t MCP_FIRST_PAGE_COMPAT_TOOL_COUNT =
    sizeof(MCP_FIRST_PAGE_COMPAT_TOOLS) / sizeof(MCP_FIRST_PAGE_COMPAT_TOOLS[0]);

static bool mcp_is_first_page_compat_tool(int source_index) {
    if (source_index < 0 || source_index >= TOOL_COUNT) {
        return false;
    }
    for (size_t i = 0; i < MCP_FIRST_PAGE_COMPAT_TOOL_COUNT; i++) {
        if (strcmp(TOOLS[source_index].name, MCP_FIRST_PAGE_COMPAT_TOOLS[i]) == 0) {
            return true;
        }
    }
    return false;
...
static int mcp_find_tool_index(const char *tool_name) {
...
/* Map a logical paginated position to the stable TOOLS source array. The
 * compatibility prefix is emitted once, then every other source entry is
 * emitted in its original order, so cursors still cover each tool exactly once. */
static int mcp_tools_list_source_index(int logical_index) {
    if (logical_index < 0 || logical_index >= TOOL_COUNT) {
        return -1;
    }

    if ((size_t)logical_index < MCP_FIRST_PAGE_COMPAT_TOOL_COUNT) {
        return mcp_find_tool_index(MCP_FIRST_PAGE_COMPAT_TOOLS[logical_index]);
    }

    int remaining = logical_index - (int)MCP_FIRST_PAGE_COMPAT_TOOL_COUNT;
    for (int source_index = 0; source_index < TOOL_COUNT; source_index++) {
        if (mcp_is_first_page_compat_tool(source_index)) {
            continue;
        }
        if (remaining == 0) {
            return source_index;
...
static char *cbm_mcp_tools_list_range(int offset, int limit, bool include_next_cursor) {
    ...
    for (int i = offset; i < end; i++) {
        int source_index = mcp_tools_list_source_index(i);
        if (source_index >= 0) {
            mcp_add_tool_def(doc, tools, source_index);
        }
    }
...
static int mcp_tools_cursor_offset(const char *params_json) {
    ...
    /* PaginatedRequestParams serializes an absent initial cursor as JSON null.
     * Treat null exactly like an omitted cursor so Codex/RMCP receives the
     * first tool page. Non-null malformed cursors still fail closed to the
     * terminal page instead of replaying page zero. */
    if (cursor && !yyjson_is_null(cursor)) {
        offset = TOOL_COUNT;
...
struct cbm_mcp_server {
    ...
    /* Global cross-project graph store — <cache>/__global__-graph.db. Manager
     * topology queries keep this distinct from the global Memory database. */
    cbm_store_t *global_graph_store; /* query-only global graph store (or NULL) */
...
void cbm_mcp_server_free(cbm_mcp_server_t *srv) {
    ...
    if (srv->global_graph_store) {
        cbm_store_close(srv->global_graph_store);
    }
    free(srv->current_project);
...
static const char *stage14_graph_shard_path(const char *uuid, char *buf, size_t bufsz) {
    char dir[CBM_SZ_1K];
    cache_dir(dir, sizeof(dir));
    snprintf(buf, bufsz, "%s/projects/%s/graph.db", dir, uuid ? uuid : "");
    return buf;
}

/* Stage 14 resolves catalog and legacy aliases to the UUID shard first. A flat
 * <project>.db is retained only as an explicit migration-period fallback. */
const char *project_db_path(const char *project, char *buf, size_t bufsz) {
    if (!cbm_validate_project_name(project)) {
        buf[0] = '\0';
        return buf;
    }
    char uuid[CBM_PROJECT_UUID_SIZE];
    if (stage14_catalog_project_uuid(project, uuid)) {
        char shard[CBM_SZ_1K];
        stage14_graph_shard_path(uuid, shard, sizeof(shard));
        if (cbm_file_exists(shard)) {
            snprintf(buf, bufsz, "%s", shard);
            return buf;
        }
    }
    char dir[CBM_SZ_1K];
...
static bool stage14_bind_graph_project(cbm_mcp_server_t *srv, const char *workspace,
                                       const char *legacy_project, char *shard_path,
...
static void merge_stage14_graph_shard(yyjson_mut_doc *doc, yyjson_mut_val *arr,
    ...
    yyjson_mut_obj_remove_key(p, "size_bytes");
    int64_t bytes = cbm_file_size(full_path);
    yyjson_mut_obj_add_int(doc, p, "size_bytes", prior + (bytes > 0 ? bytes : 0));
    yyjson_mut_obj_remove_key(p, "nodes");
    yyjson_mut_obj_add_int(doc, p, "nodes", cbm_store_count_nodes(store, internal));
    yyjson_mut_obj_remove_key(p, "edges");
    yyjson_mut_obj_add_int(doc, p, "edges", cbm_store_count_edges(store, internal));
    cbm_project_t project = {0};
    if (cbm_store_get_project(store, internal, &project) == CBM_STORE_OK) {
        if (project.root_path && project.root_path[0]) {
            yyjson_mut_obj_remove_key(p, "root_path");
            yyjson_mut_obj_add_strcpy(doc, p, "root_path", project.root_path);
            add_git_context_json(doc, p, project.root_path);
        }
...
static char *handle_list_projects(cbm_mcp_server_t *srv, const char *args) {
    ...
    char shards_root[CBM_SZ_2K];
    snprintf(shards_root, sizeof(shards_root), "%s/projects", dir_path);
...
static char *handle_index_repository(cbm_mcp_server_t *srv, const char *args) {
    ...
    char *graph_project = name_override && name_override[0] ? heap_strdup(name_override)
    ...
    if (!graph_project || !cbm_validate_project_name(graph_project) ||
        ...
        free(name_override);
        free(repo_path);
        return cbm_mcp_text_result("failed to register global project graph shard", true);
    }
    cbm_pipeline_t *p = cbm_pipeline_new(repo_path, shard_path, mode);
    if (!p) {
        free(graph_project);
        free(name_override);
        free(repo_path);
    ...
    try_artifact_bootstrap(project_name, repo_path, shard_path);
    ...
    cbm_pipeline_lock();
    stage14_set_project_index_state(srv, project_name, "indexing");
    srv->active_pipeline = p;
    int rc = cbm_pipeline_run(p);
    srv->active_pipeline = NULL;
    cbm_pipeline_unlock();
    stage14_set_project_index_state(srv, project_name, rc == 0 ? "indexed" : "failed");
...
bool cbm_mcp_memory_fixture_project_authorized(const char *project) {
    return stage11_test_project_allowlisted(project);
}
...
cbm_store_t *resolve_global_memory_store(cbm_mcp_server_t *srv, bool create) {
    ...
    if (create && srv->global_mem_store &&
        cbm_global_store_migrate(srv->global_mem_store, NULL) < CBM_STORE_OK) {
        cbm_store_close(srv->global_mem_store);
        srv->global_mem_store = NULL;
    }
    return srv->global_mem_store;
...
cbm_store_t *resolve_global_graph_store(cbm_mcp_server_t *srv) {
    if (!srv) {
        return NULL;
    }
    srv->store_last_used = time(NULL);
...
#ifdef _WIN32
    snprintf(graph_path, sizeof(graph_path), "%s\\__global__-graph.db", dir);
#else
    snprintf(graph_path, sizeof(graph_path), "%s/__global__-graph.db", dir);
#endif
    if (!cbm_file_exists(graph_path)) {
...
char *cbm_mcp_handle_tool(cbm_mcp_server_t *srv, const char *tool_name, const char *args_json) {
    ...
    char *security_block = cbm_mcp_memory_security_guard(srv, tool_name, args_json);
    if (security_block) {
    ...
    if (strcmp(tool_name, "events") == 0) {
        return handle_events(srv, args_json);
    }
    if (strcmp(tool_name, "memories_retrieve") == 0) {
        return handle_memories_retrieve(srv, args_json);
    }
    ...
    if (strcmp(tool_name, "memory_reinforcement_replay") == 0) {
        return handle_memory_reinforcement_replay(srv, args_json);
    }
    if (strcmp(tool_name, "memory_edge_lifecycle_migrate") == 0) {
        return handle_memory_edge_lifecycle_migrate(srv, args_json);
    }
    if (strcmp(tool_name, "memory_edge_maintenance") == 0) {
        return handle_memory_edge_maintenance(srv, args_json);
    }
    if (strcmp(tool_name, "memory_edge_restore") == 0) {
        return handle_memory_edge_restore(srv, args_json);
    }
    if (strcmp(tool_name, "memory_concept_generate") == 0) {
        return handle_memory_concept_generate(srv, args_json);
    }
    if (strcmp(tool_name, "memory_concept_review") == 0) {
        return handle_memory_concept_review(srv, args_json);
    }
    if (strcmp(tool_name, "memory_concept_inspect") == 0) {
        return handle_memory_concept_inspect(srv, args_json);
    }
    if (strcmp(tool_name, "memory_observe_injection") == 0) {
        return handle_memory_observe_injection(srv, args_json);
    }
    if (strcmp(tool_name, "memory_observe_usage") == 0) {
        return handle_memory_observe_usage(srv, args_json);
    }
    ...
    if (strcmp(tool_name, "admin_decay") == 0) {
        return handle_admin_decay(srv, args_json);
    }
    if (strcmp(tool_name, "memory_health") == 0) {
        return handle_memory_health(srv, args_json);
    }
    if (strcmp(tool_name, "memory_security_check") == 0) {
        return handle_memory_security_check(srv, args_json);
    }
    ...
    if (strcmp(tool_name, "memory_task_migrate") == 0) {
        return handle_memory_task_migrate(srv, args_json);
    }
