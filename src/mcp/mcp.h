/*
 * mcp.h — MCP (Model Context Protocol) server for semantic-memory-mcp.
 *
 ...
 * The long-term-memory and observe-only journal tools. Store access goes through the two
...
/* Stage 11 scope authorization. Normal operation permits only the fixed
 * project scopes. Isolated tests may use only an exact project named in the
 * test-process environment allowlist while the cache is under the OS temp dir. */
