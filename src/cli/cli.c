...
static const char skill_content[] =
    ...
    "`events`, `memories_retrieve`, `memory_update_status`, `memory_feedback`, "
    "`admin_consolidate`, `memory_health`\n"
    "\n"
    "## Edge Types\n"
...
static int verify_download_checksum(const char *archive_path, const char *archive_name) {
    ...
    if (dl_base && dl_base[0]) {
        ...
        snprintf(checksum_url, sizeof(checksum_url), "%s",
                 "https://github.com/AlbertYm/neuroplastic-memory-mcp/releases/latest/download/"
                 "checksums.txt");
    }
...
static void build_update_url(char *url, int url_sz, const char *os, const char *arch,
    ...
    if (!base_url || !base_url[0]) {
        base_url = "https://github.com/AlbertYm/neuroplastic-memory-mcp/releases/latest/download";
    }
...
static char *fetch_latest_tag(void) {
    FILE *fp = cbm_popen(
        "curl -sfI https://github.com/AlbertYm/neuroplastic-memory-mcp/releases/latest 2>/dev/null",
        "r");
    if (!fp) {
        return NULL;
    }
...
int cbm_cli_print_tool_help(const char *tool_name) {
    ...
    printf("Usage:\n");
    printf("  semantic-memory-mcp cli %s --flag value [--flag2 value2 ...]\n", tool_name);
    printf("  semantic-memory-mcp cli %s --args-file <path-to-json>\n", tool_name);
    printf("  echo '<json>' | semantic-memory-mcp cli %s\n", tool_name);
    printf("  semantic-memory-mcp cli %s '<raw-json-args>'\n", tool_name);
...
typedef struct {
    ...
    ULONGLONG extended_volume_serial;
    unsigned char extended_file_id[16];
    int extended_file_id_valid;
#else
    dev_t device;
    ino_t inode;
#endif
...
#else
    return fsync(cbm_fileno(reservation->stream));
#endif
}
...
static int global_migrate_report_discard(global_migrate_report_reservation_t *reservation) {
     ...
     * or incomplete reservation instead of risking unlinking a replacement
     * raced into the pathname; the caller emits an explicit retained warning.
     */
    (void)global_migrate_report_close(reservation);
    return -1;
#endif
}
