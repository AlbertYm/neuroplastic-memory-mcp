...
int cbm_http_parse_head(const char *data, size_t len, cbm_http_req_t *req, size_t *body_offset,
    ...
    *content_length = 0;
    ...
    while (p < head_stop) {
        ...
        if (header_name_is(p, nlen, "host")) {
            copy_header_value(colon + 1, eol, req->host, sizeof(req->host));
        } else if (header_name_is(p, nlen, "origin")) {
        ...
        } else if (header_name_is(p, nlen, "x-manager-token")) {
            copy_header_value(colon + 1, eol, req->manager_token, sizeof(req->manager_token));
