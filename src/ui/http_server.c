...
static char g_cors[1536];      /* CORS + security headers */
static char g_cors_json[1664]; /* security headers + Content-Type */

static const char *MANAGER_SECURITY_HEADERS =
...
static void update_cors(const cbm_http_req_t *req, bool manager_mode,
                        const char *exact_origin) {
    bool allowed = manager_mode ? req->origin[0] != '\0' && exact_origin &&
                                      strcmp(req->origin, exact_origin) == 0
                                : req->origin[0] != '\0' &&
                                      (cbm_http_path_match(req->origin, "http://localhost:*") ||
...
struct cbm_http_server {
    ...
    bool listener_ok;
    bool manager_mode;
    char manager_token[65];
    char exact_origin[96];
    char exact_host[64];
};

static bool manager_random_token(char output[65]) {
    ...
    bool ok = fread(bytes, 1, sizeof(bytes), random) == sizeof(bytes);
    fclose(random);
    if (!ok) return false;
#endif
...
static bool manager_token_equal(const char expected[65], const char *provided) {
    if (!provided || strlen(provided) != 64) return false;
    unsigned char diff = 0;
    for (size_t i = 0; i < 64; i++) diff |= (unsigned char)(expected[i] ^ provided[i]);
    return diff == 0;
}

static void manager_reply(cbm_http_conn_t *c, int status, const char *code) {
    cbm_http_replyf(c, status, g_cors_json,
                    "{\"schema\":\"stage12-manager-result/v1\",\"status\":\"error\","
                    "\"code\":\"%s\"}", code);
}

static bool manager_authorize(const cbm_http_server_t *srv, cbm_http_conn_t *c,
                               const cbm_http_req_t *req, bool state_changing) {
    if (!srv->manager_mode) return true;
    if (strcmp(req->host, srv->exact_host) != 0 ||
        !manager_token_equal(srv->manager_token, req->manager_token)) {
        manager_reply(c, 401, "UNAUTHORIZED");
        return false;
    }
    if (state_changing && strcmp(req->origin, srv->exact_origin) != 0) {
        manager_reply(c, 403, "ORIGIN_REJECTED");
        return false;
    }
    if (state_changing && strcmp(req->content_type, "application/json") != 0) {
        manager_reply(c, 415, "CONTENT_TYPE_REJECTED");
        return false;
    }
    return true;
}

/* Manager browser RPC intentionally exposes only the frozen Stage14-G tools.
 * The embedded MCP server also owns lifecycle/write tools, which must never be
 * reachable merely because a loopback Manager token is present in a browser. */
static bool manager_rpc_tool_allowed(const cbm_http_req_t *req) {
    ...
    yyjson_val *params = root && yyjson_is_obj(root) ? yyjson_obj_get(root, "params") : NULL;
    const char *method = root && yyjson_is_obj(root) ? yyjson_get_str(yyjson_obj_get(root, "method")) : NULL;
    const char *name = params && yyjson_is_obj(params) ? yyjson_get_str(yyjson_obj_get(params, "name")) : NULL;
    bool allowed = method && name && strcmp(method, "tools/call") == 0;
    for (size_t i = 0; allowed && i < sizeof(allowed_tools) / sizeof(allowed_tools[0]); i++) {
...
static const char *manager_body_string(yyjson_val *root, const char *key) {
    yyjson_val *value = root ? yyjson_obj_get(root, key) : NULL;
    return value && yyjson_is_str(value) ? yyjson_get_str(value) : NULL;
}

static void manager_handle_get(cbm_http_conn_t *c, const cbm_http_req_t *req,
                               bool tasks) {
    char project[256];
    if (!cbm_http_query_param(req->query, "project", project, sizeof(project))) {
        manager_reply(c, 400, "INVALID_PROJECT");
        return;
    }
    char *json = tasks ? cbm_admin_tasks_json(project, 50)
                       : cbm_admin_health_json(project, CBM_VERSION);
    if (!json) {
        manager_reply(c, 500, "MANAGER_ERROR");
        return;
    }
...
static void manager_handle_post(cbm_http_conn_t *c, const cbm_http_req_t *req,
                                bool restore) {
    yyjson_doc *doc = req->body ? yyjson_read(req->body, req->body_len, 0) : NULL;
    yyjson_val *root = doc ? yyjson_doc_get_root(doc) : NULL;
    const char *project = manager_body_string(root, "project");
    const char *destination = manager_body_string(root, restore ? "target_directory" : "destination_root");
    const char *source = restore ? manager_body_string(root, "source_directory") : NULL;
    if (!root || !project || !destination || (restore && !source)) {
        yyjson_doc_free(doc);
        manager_reply(c, 400, "INVALID_ARGUMENT");
        return;
    }
    char *json = restore ? cbm_admin_verify_restore_json(project, source, destination)
                         : cbm_admin_create_backup_json(project, destination);
    yyjson_doc_free(doc);
    if (!json) {
        manager_reply(c, 500, "MANAGER_ERROR");
        return;
...
static void dispatch_request(cbm_http_server_t *srv, cbm_http_conn_t *c,
                             const cbm_http_req_t *req) {
    update_cors(req, srv->manager_mode, srv->exact_origin);
    ...
    if (srv->manager_mode && is_get &&
        cbm_http_path_match(req->path, "/api/manager/health*")) {
        if (manager_authorize(srv, c, req, false)) manager_handle_get(c, req, false);
        return;
    }
    if (srv->manager_mode && is_get &&
    ...
    if (srv->manager_mode && is_post &&
        cbm_http_path_match(req->path, "/api/manager/backup")) {
        if (manager_authorize(srv, c, req, true)) manager_handle_post(c, req, false);
        return;
    }
    if (srv->manager_mode && is_post &&
        cbm_http_path_match(req->path, "/api/manager/verify-restore")) {
        if (manager_authorize(srv, c, req, true)) manager_handle_post(c, req, true);
        return;
    }

    if (cbm_http_path_match(req->path, "/api/process-kill")) {
        manager_reply(c, 403, "FEATURE_DISABLED");
        return;
    }

    if (srv->manager_mode && is_post && cbm_http_path_match(req->path, "/rpc")) {
        if (!manager_authorize(srv, c, req, true)) return;
        if (!manager_rpc_tool_allowed(req)) {
            manager_reply(c, 403, "FEATURE_DISABLED");
            return;
        }
    ...
    if (srv->manager_mode &&
        (cbm_http_path_match(req->path, "/api/*") || cbm_http_path_match(req->path, "/rpc"))) {
        if (!manager_authorize(srv, c, req, is_post)) return;
        manager_reply(c, 403, "FEATURE_DISABLED");
        return;
    }
...
static cbm_http_server_t *http_server_new_mode(int port, bool manager_mode) {
    cbm_http_server_t *srv = calloc(1, sizeof(*srv));
    if (!srv)
        return NULL;

    srv->port = manager_mode ? 0 : port;
    srv->manager_mode = manager_mode;
    if (manager_mode && !manager_random_token(srv->manager_token)) {
        free(srv);
        return NULL;
    }
    ...
    srv->listener = cbm_httpd_listen(srv->port);
    if (!srv->listener) {
        char port_str[16];
    ...
    snprintf(srv->exact_host, sizeof(srv->exact_host), "127.0.0.1:%d", srv->port);
    snprintf(srv->exact_origin, sizeof(srv->exact_origin), "http://%s", srv->exact_host);
...
cbm_http_server_t *cbm_http_server_new(int port) {
    return http_server_new_mode(port, false);
}

cbm_http_server_t *cbm_http_server_new_manager(void) {
    return http_server_new_mode(0, true);
}

bool cbm_http_server_manager_url(const cbm_http_server_t *srv, char *out, size_t outsz) {
    if (!srv || !srv->manager_mode || !srv->listener_ok || !out || outsz == 0) return false;
    int written = snprintf(out, outsz, "%s/#token=%s", srv->exact_origin, srv->manager_token);
...
void cbm_http_server_free(cbm_http_server_t *srv) {
    ...
    cbm_mcp_server_free(srv->mcp);
    memset(srv->manager_token, 0, sizeof(srv->manager_token));
    free(srv);
}
...
void cbm_http_server_run(cbm_http_server_t *srv) {
    ...
    while (!atomic_load(&srv->stop_flag)) {
        ...
        if (rc == 0) {
            ...
            cbm_http_replyf(conn, rc, srv->manager_mode ? MANAGER_SECURITY_HEADERS : "",
                            "bad request");
