...
TEST(log_level_default) {
    /* Stage 11 defaults application logs to WARN. */
    ASSERT_EQ(cbm_log_get_level(), CBM_LOG_WARN);
    PASS();
}

TEST(log_omits_sensitive_fields) {
    cbm_log_set_level(CBM_LOG_DEBUG);
    cbm_log_set_format(CBM_LOG_FORMAT_JSON);
    capture_start();
    cbm_log_warn("security.test", "code", "SECURITY_SECRET_REJECTED", "content",
                 "raw-sensitive-value", "payload", "raw-payload-value", "content_sha256",
                 "abc123");
    const char *output = capture_end();
    cbm_log_set_format(CBM_LOG_FORMAT_TEXT);
    cbm_log_set_level(CBM_LOG_INFO);

    ASSERT_NOT_NULL(strstr(output, "SECURITY_SECRET_REJECTED"));
    ASSERT_NOT_NULL(strstr(output, "content_sha256"));
    ASSERT_NULL(strstr(output, "raw-sensitive-value"));
    ASSERT_NULL(strstr(output, "raw-payload-value"));
    ASSERT_NULL(strstr(output, "\"content\""));
    ASSERT_NULL(strstr(output, "\"payload\""));
    PASS();
}

TEST(log_security_event_fixed_fields) {
    cbm_log_set_level(CBM_LOG_DEBUG);
    cbm_log_set_format(CBM_LOG_FORMAT_TEXT);
    capture_start();
    cbm_log_security_event("SECURITY_SECRET_REJECTED", "events", "scopehash", "contenthash",
                           42, 7);
    const char *output = capture_end();
    cbm_log_set_level(CBM_LOG_INFO);

    ASSERT_NOT_NULL(strstr(output, "msg=memory.security"));
    ASSERT_NOT_NULL(strstr(output, "code=SECURITY_SECRET_REJECTED"));
    ASSERT_NOT_NULL(strstr(output, "operation=events"));
    ASSERT_NOT_NULL(strstr(output, "content_sha256=contenthash"));
    ASSERT_NOT_NULL(strstr(output, "content_length=42"));
    ASSERT_NOT_NULL(strstr(output, "duration_ms=7"));
    PASS();
}
...
TEST(log_redacts_windows_absolute_filesystem_paths) {
    cbm_log_set_level(CBM_LOG_DEBUG);
    cbm_log_set_format(CBM_LOG_FORMAT_TEXT);
    capture_start();
    cbm_log_warn("path.test", "path", "C:\\Users\\Example\\private\\store.db", "file",
                 "D:/private/config.json", "route", "/api/layout");
    const char *output = capture_end();
    cbm_log_set_level(CBM_LOG_INFO);

    ASSERT_NOT_NULL(strstr(output, "path=[absolute-path-redacted]"));
    ASSERT_NOT_NULL(strstr(output, "file=[absolute-path-redacted]"));
    ASSERT_NULL(strstr(output, "C:\\Users\\Example"));
    ASSERT_NULL(strstr(output, "D:/private/config.json"));
    ASSERT_NOT_NULL(strstr(output, "route=/api/layout"));
    PASS();
}
...
SUITE(log) {
    ...
    RUN_TEST(log_omits_sensitive_fields);
    RUN_TEST(log_security_event_fixed_fields);
    ...
    RUN_TEST(log_redacts_windows_absolute_filesystem_paths);
