...
This page documents the configuration files that `semantic-memory-mcp` reads or writes today.

## At a Glance

| Purpose | Path | Format | Notes |
|---|---|---|---|
| Global custom extension mapping | `$XDG_CONFIG_HOME/semantic-memory-mcp/config.json` | JSON | Falls back to `~/.config/semantic-memory-mcp/config.json` when `XDG_CONFIG_HOME` is unset. |
...
| CLI-managed runtime settings | `${CBM_CACHE_DIR:-~/.cache/semantic-memory-mcp}/_config.db` | SQLite | Written by `semantic-memory-mcp config set/reset`. |
| UI settings | `${CBM_CACHE_DIR:-~/.cache/semantic-memory-mcp}/config.json` | JSON | Stores `ui_enabled` and `ui_port`. |
...
```text
$XDG_CONFIG_HOME/semantic-memory-mcp/config.json
```

Fallback when `XDG_CONFIG_HOME` is unset:

```text
~/.config/semantic-memory-mcp/config.json
```
...
```text
${CBM_CACHE_DIR:-~/.cache/semantic-memory-mcp}/_config.db
```
...
```text
${CBM_CACHE_DIR:-~/.cache/semantic-memory-mcp}/config.json
```
...
|---|---|---|
| `CBM_CACHE_DIR` | `~/.cache/semantic-memory-mcp` | Override the cache directory used for indexes, `_config.db`, and UI `config.json`. |
...
```bash
semantic-memory-mcp install --dry-run
```
