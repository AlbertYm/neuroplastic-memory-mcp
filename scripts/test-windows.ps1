<#
.SYNOPSIS
    Run the native-Windows product-surface test suite for semantic-memory-mcp.

.DESCRIPTION
    Builds the product binary (build/c/semantic-memory-mcp.exe) if it is not
    already present, then runs the deterministic Windows integration tests under
    tests/windows/ against a real semantic-memory-mcp.exe (real stdio / CLI /
...
.PARAMETER Binary
    Path to an existing semantic-memory-mcp.exe. If omitted, the script builds it
...
.EXAMPLE
    pwsh -File scripts/test-windows.ps1 -GuardsOnly -Binary build\c\semantic-memory-mcp.exe
#>
[CmdletBinding()]
param(
    [string]$Binary,
...
function Resolve-Binary {
    ...
    $built = Join-Path $repoRoot "build\c\semantic-memory-mcp.exe"
