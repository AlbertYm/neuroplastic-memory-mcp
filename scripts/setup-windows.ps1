# semantic-memory-mcp setup script (Windows)
...
function Configure-ClaudeCode($McpConfig) {
    Write-Host ""
    $answer = Read-Host "Configure Claude Code to use semantic-memory-mcp? [y/N]"
    ...
    if ($answer -match '^[Yy]$') {
        ...
        $settings["mcpServers"]["semantic-memory-mcp"] = $McpConfig
        ...
        Write-Host ""
        $snippet = @{ mcpServers = @{ "semantic-memory-mcp" = $McpConfig } }
...
Write-Host ""
Write-Host "semantic-memory-mcp installer (Windows)" -ForegroundColor White
Write-Host ""
...
if ($FromSource) {
    ...
    # Clone or update
    Write-Host ""
    $sourceDir = "/home/$wslUser/.local/share/semantic-memory-mcp"
    try {
    ...
    Invoke-WSL "mkdir -p /home/$wslUser/.local/bin && cd $sourceDir && scripts/build.sh && cp build/c/semantic-memory-mcp $wslBinaryPath"
    ...
    Write-Host "    wsl.exe -- rm -rf ~/.cache/semantic-memory-mcp/"
    ...
    $release = Invoke-RestMethod -Uri $releaseUrl -Headers @{ "User-Agent" = "semantic-memory-mcp-setup" }
    ...
    $asset = "semantic-memory-mcp-windows-amd64.zip"
    ...
    Write-Host "    Remove-Item -Recurse -Force `"$env:LOCALAPPDATA\semantic-memory-mcp`"  # graph database"
}
