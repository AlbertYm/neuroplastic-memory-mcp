...
#
# A comprehensive, fast, portable smoke battery for the semantic-memory-mcp
...
#
# Usage:
#   scripts/smoke-invariants.sh <binary>        # e.g. build/c/semantic-memory-mcp(.exe)
#
# Portability notes:
...
inv_help() {
    ...
    if printf '%s' "$RB_OUT" | grep -qiE 'usage|semantic-memory-mcp'; then
        pass "help"
    else
