...
if command -v pgrep &>/dev/null && [ "$(uname)" != "MINGW64_NT" ] 2>/dev/null; then
  ...
  sleep 1
  RESIDUAL=$(pgrep -f "semantic-memory-mcp.*cli" 2>/dev/null | wc -l | tr -d ' \n' || echo "0")
  RESIDUAL="${RESIDUAL:-0}"
  if [ "$RESIDUAL" -gt 0 ]; then
    echo "WARNING: $RESIDUAL residual semantic-memory-mcp process(es) found"
  else
...
cp "$BINARY" "$INSTALL_DIR/semantic-memory-mcp"
chmod 755 "$INSTALL_DIR/semantic-memory-mcp"

# Verify installed binary works
INSTALLED_VER=$("$INSTALL_DIR/semantic-memory-mcp" --version 2>&1)
...
cp "$BINARY" "$REPLACE_DIR/smoke-semantic-memory-mcp"

# 3. Simulate cbm_replace_binary: unlink old, copy new
rm -f "$INSTALL_DIR/semantic-memory-mcp"
cp "$REPLACE_DIR/smoke-semantic-memory-mcp" "$INSTALL_DIR/semantic-memory-mcp"
chmod 755 "$INSTALL_DIR/semantic-memory-mcp"

# 4. Verify replaced binary works
REPLACED_VER=$("$INSTALL_DIR/semantic-memory-mcp" --version 2>&1)
...
chmod 444 "$INSTALL_DIR/semantic-memory-mcp"
rm -f "$INSTALL_DIR/semantic-memory-mcp"
cp "$REPLACE_DIR/smoke-semantic-memory-mcp" "$INSTALL_DIR/semantic-memory-mcp"
chmod 755 "$INSTALL_DIR/semantic-memory-mcp"
READONLY_VER=$("$INSTALL_DIR/semantic-memory-mcp" --version 2>&1)
...
if [[ "$BINARY" == *.exe ]]; then
  cp "$BINARY" "$FAKE_HOME/.local/bin/semantic-memory-mcp.exe"
  SELF_PATH="$FAKE_HOME/.local/bin/semantic-memory-mcp.exe"
else
  cp "$BINARY" "$FAKE_HOME/.local/bin/semantic-memory-mcp"
  SELF_PATH="$FAKE_HOME/.local/bin/semantic-memory-mcp"
fi
...
CMD=$(json_get "$FAKE_HOME/.claude.json" "d.get('mcpServers',{}).get('semantic-memory-mcp',{}).get('command','')")
...
CMD=$(json_get "$FAKE_HOME/.claude/.mcp.json" "d['mcpServers']['semantic-memory-mcp']['command']")
...
# 8f-8h: Codex TOML
if ! grep -q '\[mcp_servers.semantic-memory-mcp\]' "$FAKE_HOME/.codex/config.toml"; then
...
if [ ! -f "$FAKE_HOME/.codex/AGENTS.md" ] || ! grep -q 'semantic-memory-mcp' "$FAKE_HOME/.codex/AGENTS.md"; then
...
CMD=$(json_get "$FAKE_HOME/.gemini/settings.json" "d['mcpServers']['semantic-memory-mcp']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
...
if [ -f "$ZED_CFG" ]; then
  CMD=$(json_get "$ZED_CFG" "d['context_servers']['semantic-memory-mcp']['command']")
  if ! path_match "$CMD" "$SELF_PATH"; then
...
CMD=$(json_get "$FAKE_HOME/.config/opencode/opencode.json" "d['mcp']['semantic-memory-mcp']['command'][0]")
if [ -n "$CMD" ]; then
  if ! path_match "$CMD" "$SELF_PATH"; then
...
CMD=$(json_get "$FAKE_HOME/.gemini/config/mcp_config.json" "d['mcpServers']['semantic-memory-mcp']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
...
if [ -f "$FAKE_HOME/CONVENTIONS.md" ]; then
  if ! grep -q 'semantic-memory-mcp' "$FAKE_HOME/CONVENTIONS.md"; then
...
CMD=$(json_get "$KILO_CFG" "d['mcpServers']['semantic-memory-mcp']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
...
if [ ! -f "$FAKE_HOME/.kilocode/rules/semantic-memory-mcp.md" ]; then
...
CMD=$(json_get "$VSCODE_CFG" "d['servers']['semantic-memory-mcp']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
...
# 8w: OpenClaw MCP
CMD=$(json_get "$FAKE_HOME/.openclaw/openclaw.json" "d['mcpServers']['semantic-memory-mcp']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
...
import json, sys
d = json.load(sys.stdin)
if 'semantic-memory-mcp' in d.get('mcpServers', {}):
    sys.exit(1)
...
import json, sys
d = json.load(sys.stdin)
sys.exit(1 if 'semantic-memory-mcp' in d.get('mcpServers', {}) else 0)
" 2>/dev/null; then
...
if grep -q '\[mcp_servers.semantic-memory-mcp\]' "$FAKE_HOME/.codex/config.toml" 2>/dev/null; then
...
import json, sys
d = json.load(sys.stdin)
has_mcp = 'semantic-memory-mcp' in d.get('mcpServers', {})
has_existing = d.get('existingKey', False)
hooks = d.get('hooks', {}).get('BeforeTool', [])
has_hook = any('semantic-memory-mcp' in str(h) for h in hooks)
...
import json, sys
d = json.load(sys.stdin)
sys.exit(1 if 'semantic-memory-mcp' in d.get('servers', {}) else 0)
" 2>/dev/null; then
...
cp "$BINARY" "$IDEM_HOME/.local/bin/semantic-memory-mcp"
...
import json, sys
d = json.load(sys.stdin)
print(list(d.get('mcpServers',{}).keys()).count('semantic-memory-mcp'))
" 2>/dev/null || echo "0")
if [ "$COUNT" != "1" ]; then
...
cp "$BINARY" "$CORRUPT_HOME/.local/bin/semantic-memory-mcp"
...
cp "$BINARY" "$DBL_HOME/.local/bin/semantic-memory-mcp"
...
SECURITY_BIN="$SECURITY_DIR/semantic-memory-mcp"
...
if [ -n "${SMOKE_DOWNLOAD_URL:-}" ]; then
  ...
  if [[ "$BINARY" == *.exe ]]; then
    cp "$BINARY" "$UPDATE_HOME/.local/bin/semantic-memory-mcp.exe"
    chmod 755 "$UPDATE_HOME/.local/bin/semantic-memory-mcp.exe"
  else
    cp "$BINARY" "$UPDATE_HOME/.local/bin/semantic-memory-mcp"
    chmod 755 "$UPDATE_HOME/.local/bin/semantic-memory-mcp"
    if [ "$(uname -s)" = "Darwin" ]; then
      codesign --sign - --force "$UPDATE_HOME/.local/bin/semantic-memory-mcp" 2>/dev/null || true
    fi
  fi

  # Pre-install agent config with a WRONG binary path (simulates stale config)
  echo '{"mcpServers":{"semantic-memory-mcp":{"command":"/old/stale/path"}}}' > "$UPDATE_HOME/.claude.json"
  ...
  if [ ! -f "$UPDATE_HOME/.local/bin/semantic-memory-mcp" ]; then
    echo "FAIL 14b: binary missing after update"
    exit 1
  fi
  UPD_BIN="$UPDATE_HOME/.local/bin/semantic-memory-mcp"
  if [ "$(uname -s)" = "Darwin" ]; then
  ...
  UPD_CMD=$(cat "$UPDATE_HOME/.claude.json" 2>/dev/null | python3 -c "import json,sys; d=json.load(sys.stdin); print(d.get('mcpServers',{}).get('semantic-memory-mcp',{}).get('command',''))" 2>/dev/null || echo "")
  ...
  if [ -f "$UPDATE_HOME/.local/bin/semantic-memory-mcp" ] || [ -f "$UPDATE_HOME/.local/bin/semantic-memory-mcp.exe" ]; then
...
import json, sys
d = json.load(sys.stdin)
if 'semantic-memory-mcp' in d.get('mcpServers', {}): sys.exit(1)
sys.exit(0)
" 2>/dev/null; then
  ...
  cp "$BINARY" "$UPDATE_DIR/install/semantic-memory-mcp"
  chmod 755 "$UPDATE_DIR/install/semantic-memory-mcp"
  cp "$BINARY" "$UPDATE_DIR/smoke-downloaded"
  rm -f "$UPDATE_DIR/install/semantic-memory-mcp"
  cp "$UPDATE_DIR/smoke-downloaded" "$UPDATE_DIR/install/semantic-memory-mcp"
  chmod 755 "$UPDATE_DIR/install/semantic-memory-mcp"
  if [ "$(uname -s)" = "Darwin" ]; then
    codesign --sign - --force "$UPDATE_DIR/install/semantic-memory-mcp" 2>/dev/null || true
  fi
  if ! "$UPDATE_DIR/install/semantic-memory-mcp" --version > /dev/null 2>&1; then
...
DL_ARCHIVE="semantic-memory-mcp-${DL_OS}-${DL_ARCH}.${DL_EXT}"
DL_ARCHIVE_UI="semantic-memory-mcp-ui-${DL_OS}-${DL_ARCH}.${DL_EXT}"
...
DL_BIN="$DL_DIR/semantic-memory-mcp"
if [ ! -f "$DL_BIN" ]; then
...
if [ "$DL_OS" != "windows" ] && [ -f "$REPO_ROOT/install.sh" ]; then
  ...
  # 13b: binary placed
  if [ ! -f "$INSTALL_TEST_DIR/semantic-memory-mcp" ]; then
  ...
  if [ "$DL_OS" = "darwin" ]; then
    codesign --sign - --force "$INSTALL_TEST_DIR/semantic-memory-mcp" 2>/dev/null || true
  fi
  if ! "$INSTALL_TEST_DIR/semantic-memory-mcp" --version > /dev/null 2>&1; then
  ...
  if [ "$DL_OS" = "darwin" ]; then
    if codesign -v "$INSTALL_TEST_DIR/semantic-memory-mcp" 2>/dev/null; then
  ...
  if [ -f "$INSTALL_TEST_HOME/.claude.json" ] && grep -q 'semantic-memory-mcp' "$INSTALL_TEST_HOME/.claude.json" 2>/dev/null; then
  ...
  # 13g: binary placed
  PS1_BIN="$PS1_TEST_DIR/semantic-memory-mcp.exe"
  if [ ! -f "$PS1_BIN" ] && [ -f "$PS1_TEST_DIR/semantic-memory-mcp" ]; then
    PS1_BIN="$PS1_TEST_DIR/semantic-memory-mcp"
  fi
  if [ -f "$PS1_BIN" ]; then
