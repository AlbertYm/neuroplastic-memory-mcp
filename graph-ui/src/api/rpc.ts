...
function captureManagerToken() {
  ...
  if (/^[0-9a-f]{64}$/.test(token)) managerToken = token;
...
export async function managerFetch(input: string, init: RequestInit = {}) {
  if (!managerToken && window.location.hash) captureManagerToken();
  const headers = new Headers(init.headers);
  if (managerToken) headers.set("X-Manager-Token", managerToken);
...
export async function callTool<T = unknown>(
  ...
  const res = await managerFetch("/rpc", {
    method: "POST",
